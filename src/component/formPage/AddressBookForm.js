...
class AddressBookForm extends Component {
	constructor(props) {
		...
		this.state = {
			...
			nameError: '', //here we have to define state
			id: '',
			isUpdated: false
		};
	...
	componentDidMount() {
		...
		AddressBookService.getAddressById(this.props.addressId).then((result) => {
			this.setState({
				...
				phone: result.data.data.phone //phone no
			});
		});
	}
	...
	handleChange(event) {
		if (event.target.firstname === 'firstname') {
	...
	// handleChange(event) {
	// 	if (event.target.Lastname === 'Lastname') {
	// 		let Lastname = RegExp('^[A-Z]{1}[a-z]{2,}[ ][A-Z]{1}[a-z]{2,}$');
	// 		if (Lastname.test(event.target.value)) this.setState({ nameError: '' });
	// 		else this.setState({ nameError: 'Invalid name' });
	// 	}
